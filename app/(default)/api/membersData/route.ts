...
export async function POST(request: Request) {
  try {
    ...
    if (!year) {
      return NextResponse.json(
        { message: "Missing Year.", error: true },
        { status: 400 }
      );
    }
    if (!role) {
      return NextResponse.json(
        { message: "Missing Role.", error: true },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: `Failed to add member: ${error.message}`, error: true },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    if (imageUrl && imageUrl !== memberData.imageUrl) {
      ...
      const imageRef = ref(
        storage,
        `members/${name}_${timestamp}.${extension}`
      );
      await uploadBytes(imageRef, blob, { contentType });
      downloadURL = await getDownloadURL(imageRef);
    ...
    return NextResponse.json(
      { message: `Failed to update member: ${error.message}`, error: true },
      { status: 500 }
    );
  }
}
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json(
      { message: `Failed to delete member: ${error.message}`, error: true },
      { status: 500 }
    );
  }
}
